...
class G2SDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> G2SBatch:
        ...
        # fnode和fmess记录着这个batch中所有的feature
        # agraph、bgraph同样记录者所有的邻接矩阵
        # atom_scope和bond_scope是记录者起始位置和长度
        '''
        graph_features[0]:
        ...
        a_feature: shape [59,9]     9 non one-hot
        b_feature: shape [132,11]   2 + 9 one-hot       132 = (24 + 42) * 2 其中b_feature前两个元素相反的点,后面的元素都相等

        a_graph:    shape[59,11] 邻接矩阵
        b_graph:    shape[132,11] 邻接矩阵
        '''
        fnode, fmess, agraph, bgraph, atom_scope, bond_scope = collate_graph_features(graph_features)

        '''
        fnode:      shape [1325,105]    这个batch一共1325个原子 one-hot
        fmess:      shape [2903,11]     这个batch一共2903个边   前两个值加了下标
        agraph:     shape [1325,5]
        bgraph:     shape [2903,4]      5和4为这个batch中可能存在长度的最大值,其实就是把a_graph、b_graph进行了精简
        atom_scope: shape [27,2]        装着某原子的起始位置和长度
        bond_scope: shape [27,2]
        '''
        # target (seq)
        tgt_token_ids = self.tgt_token_ids[data_indices]
        tgt_lengths = self.tgt_lens[data_indices]
