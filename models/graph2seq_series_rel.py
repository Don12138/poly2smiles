...
class Graph2SeqSeriesRel(nn.Module):
    ...
    def forward(self, reaction_batch: G2SBatch):
        ...
        regression_loss = self.numerical_criterion(
            x=dec_outs,
        ...
        return loss, acc, classification_loss, regression_loss

    def predict_step(self, reaction_batch: G2SBatch,
                     batch_size: int, beam_size: int, n_best: int, temperature: float,
                     min_length: int, max_length: int, target_prefix = None, generate = False,std_dev=0.01) -> Dict[str, Any]:
        if beam_size == 1:
        ...
        if generate:
            noise = torch.randn_like(padded_memory_bank)
            padded_memory_bank = padded_memory_bank + noise * std_dev
        ...
        #memory_bank 和 memory_lengths 就是把上面二者复制成了5个

        
        # (3) Begin decoding step by step:
        for step in range(decode_strategy.max_length):
            #decoder_input就是目前的预测值
