...
class DGCNEncoder(nn.Module):
    ...
    def forward(self, fnode: torch.Tensor, fmess: torch.Tensor,
        ...
        # pdb.set_trace()
        h = self.rnn(fmess, bgraph)                     # [batch_bond_num , encoder_hidden_size=256]
        nei_message = index_select_ND(h, 0, agraph)     # [batch_atom_num , atom_graph_element_len = 5 , encoder_hidden_size=256]
        nei_message = nei_message.sum(dim=1)            # [batch_atom_num , encoder_hidden_size=256]
        node_hiddens = torch.cat([fnode, nei_message], dim=1)# [batch_atom_num , encoder_hidden_size + fnode_size]
        node_hiddens = self.W_o(node_hiddens)           # [batch_atom_num , encoder_hidden_size=256]
