...
def main(args):
    ...
    model = model_class(args, vocab, property_info)
    ...
    total_step = 0
    accum = 0
    losses, accs, classification_losses, regression_losses = [], [] ,[] ,[]
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.autograd.profiler.profile(enabled=args.do_profile,
                ...
                with torch.cuda.amp.autocast(enabled=args.enable_amp):
                    loss, acc, classification_loss, regression_loss = model(batch)
            ...
            if (accum == 0) and (total_step > 0) and (total_step % args.log_iter == 0):
                logging.info(f"Step {total_step}, loss: {np.mean(losses)}, acc: {np.mean(accs)}, "
                             f"Classification loss: {np.mean(classification_losses)}, Regression loss: {np.mean(regression_losses)}, "
            ...
            if (accum == 0) and (total_step > 0) and (total_step % args.eval_iter == 0):
                ...
                with torch.no_grad():
                    for eval_idx, eval_batch in enumerate(valid_loader):
                        ...
                        eval_loss, eval_acc, classification_loss, regression_loss = model(eval_batch)
                        eval_meters[0] += eval_loss.item() / eval_count
                        eval_meters[1] += eval_acc * 100 / eval_count
                        eval_meters[2] += classification_loss.item()/ eval_count
                        eval_meters[3] += regression_loss.item() / eval_count

                logging.info(f"Evaluation (with teacher) at step {total_step}, eval loss: {eval_meters[0]}, "
                             f"Classification loss: {eval_meters[2]}, Regression loss: {eval_meters[3]}"
